...
function render(filter="all",q=""){
 ...
 cards.innerHTML=list.map(x=>`<article class="card"><div class="pic">${x.emoji}</div><div class="info"><div class="name">${x.name}</div><div class="meta">${x.meta}</div><span class="price">${x.price.toLocaleString()} ₸</span><button class="add" onclick="add(${x.id})">+</button></div></article>`).join("");
}
...
function showCart(){
 openModal(`<h2>🛒 Корзина</h2>${cart.length?cart.map(x=>`<p>${x.emoji} ${x.name} — ${x.price.toLocaleString()} ₸</p>`).join(""):`<p>Корзина пока пустая.</p>`}${cart.length?`<hr><b>Итого: ${cart.reduce((a,x)=>a+x.price,0).toLocaleString()} ₸</b><button class="checkout" onclick="checkout()">Оформить заказ</button>`:""}`);
}
function checkout(){

...
let dishes=[];
let cart=[];
async function loadDishes(){const {data,error}=await supabase.from("dishes").select("*");if(error){console.error(error);return;}dishes=data||[];render();}if(error){console.error(error);return;}dishes=data||[];render();}if(error){console.error(error);return;}dishes=data||[];render();}
